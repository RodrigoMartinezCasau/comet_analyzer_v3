#include <stdio.h>
#include <string.h>
#include <math.h> // We need the math library for the gausian probability
#include "spectrum_analyzer.h"

// Gaussian function to calculate the probability of each element
static double gaussian_probability(double diff, double sigma) {
    return exp(-(diff * diff) / (2 * sigma * sigma));
}

// We search for peaks with the help of the intensity values from the DataPoint 
...
// Based on the peaks we have found we try to match certain peaks with potential elements given its wavelength 
